...
Push.Configure = serverConfig => {
  ...
  const querySend = (query, mongoNote) => {
    ...
    query.rootUrl = process.env.ROOT_URL;
    ...
    Push.appCollection.find(query).map(app => {
      ...
      Object.keys(app.token).forEach(key => {
        self.sendNotification(app.token[key], mongoNote)
      })
    ...
    if (isDebug) {
      ...
      if (!countApn.length && !countAndroid.length && !countWeb.length) {
        ...
        if (!Push.appCollection.findOne({ 'token.apn': { $exists: true }, rootUrl: process.env.ROOT_URL })) {
          console.log('Push, GUIDE: The "Push.appCollection" - No APN clients have registred on the server yet...')
        }
      } else if (!countAndroid.length) {
        if (!Push.appCollection.findOne({ 'token.android': { $exists: true }, rootUrl: process.env.ROOT_URL })) {
        ...
        if (!Push.appCollection.findOne({ 'token.web': { $exists: true }, rootUrl: process.env.ROOT_URL })) {
  ...
  if (serverConfig.defaults && serverConfig.defaults.sendInterval !== null) {
    const sendNotification = mongoNote => {
      ...
      console.log("--- sendNotification mongoNote", mongoNote)
