...
const isDebug = true; // process.env.PUSH_DEBUG === 'true'
...
Push.Configure = serverConfig => {
  ...
  self.serverSend = mongoNote => {
    console.log('Push: Send message "' + mongoNote.title + '" to user(s): ', mongoNote.userIds);
  ...
  if (serverConfig?.defaults?.sendInterval !== null) {
    const processSendNotification = mongoNote => {
      ...
      if (reserved) {
        ...
        if (!serverConfig?.defaults?.keepNotifications) {
          // console.log('Push: Removing notification:', mongoNote);
    ...
    sendWorker(() => {
      // console.log("Push: Send worker started, isSendingNotification", isSendingNotification, serverConfig?.defaults?.sendInterval);
      ...
      try {
        ...
        Push.notifications.find({
          ...
          try {
            console.log("sendWorker mongoNote", mongoNote);
