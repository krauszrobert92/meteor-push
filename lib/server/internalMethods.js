...
Meteor.methods({
  'push-update': function (options) {
    ...
    if (options.userId) {
      doc = Push.appCollection.findOne({ userId: options.userId, rootUrl })
    }
    ...
    if (!doc) {
      ...
      for (const key in options.token)
        token[key] = options.token[key];


      Push.appCollection.update({ _id: doc._id, rootUrl }, {
        $set: {
          updatedAt: Date.now(),
          token,
        }
  ...
  'push-setuser': function (pushTokenDBId) {
    ...
    const found = Push.appCollection.update({ _id: pushTokenDBId, rootUrl: process.env.ROOT_URL }, { $set: { userId: this.userId } }, (err, res) => {
      if (err) {
  ...
  'push-metadata': function (data) {
    ...
    // Set the metadata
    const found = Push.appCollection.update({ _id: data.id, rootUrl: process.env.ROOT_URL }, { $set: { metadata: data.metadata } })
  ...
  'push-enable': function (data) {
    ...
    const found = Push.appCollection.update({ _id: data.id, rootUrl: process.env.ROOT_URL }, { $set: { enabled: data.enabled } })
  ...
  'push-check-is-enabled': function (data) {
    ...
    const found = Push.appCollection.findOne({ _id: data.id, rootUrl: process.env.ROOT_URL, enabled: { $exists: true } }, { fields: { enabled: 1 } })
  ...
  'push-unsub-webpush': function (pushTokenId) {
    ...
    return Push.appCollection.remove({ _id: pushTokenId, rootUrl: process.env.ROOT_URL })
  }
})
