              ...
              if (Meteor.isClient && !Meteor.isCordova) {
                getToken(this.push, { vapidKey: configuration.publicVapidKey }).then(newToken => {
                  ...
                  if (newToken) { // && (!storedToken || storedToken !== newToken)
                    ...
                    console.log("Calling token-insert", tokenData);

                    Meteor.call('token-insert', tokenData, (err, res) => {
                    }, (error, result) => {
                      console.log("token-insert result", error, result);
                      // if (!error)
                      //   deviceStorage.setItem('Push.tokenId', result) // _id of the document in Mongo
                    });

                    // Meteor.callAsync('token-insert', tokenData)
                    //   .then(res => {
                    //     console.log("token-insert", res);
                    ...
                    //   })
                    //   .catch(err => {
                    //     console.warn('token-insert, Could not save this token to DB: ', err)
